...
#define USER_EMAIL "bjn@gmail.com"
#define USER_PASSWORD "zxcv123"
...
String documentPath = "system_collection/AeroHouse_6.0";
...
unsigned long misterOffTime = 180000;
...
void loop() {
  ...
  if(currentHour<8||currentHour>18){ //turn off LED if its not 7AM-5PM
  ...
  if(!wifiIsConnected(currentMillis)){
    switchToAPMode();
  }
}
...
float phRead(){
  ...
  return ((2.50 - voltage) / 0.18) - 7.5;
}

void mistMakerControl(unsigned long currentMillis){

  if(!mistMakerState){
    toggleMistMakers(true);
...
void toggleMistMakers(bool state){
  digitalWrite(MIST_PIN, state);
}

void LedControl(){
  if(redLedState)  { digitalWrite(RED_PIN, HIGH);  }
  else             { digitalWrite(RED_PIN, LOW);   }
  if(blueLedState) { digitalWrite(BLUE_PIN, HIGH); }
...
void paramReading(unsigned long currentMillis){
  if(currentMillis - paramReadingMillis >= paramReadingInterval){
    ...
    if(currentMillis - averageReadingMillis >= averageReadingInterval){
      averageReadingMillis = currentMillis;
      ...
      averagePH = (int)((readingsSumPH / readingsCount)*10) / 10.0;
...
// --------------------- WIFI CHECKING / CONNECTING --------------------- //


bool wifiIsConnected(unsigned long currentMillis){
  if(currentMillis - wifiCheckMillis >= paramReadingMillis){
    if (Firebase.ready()){
      return true;
    }
    return false;
...
void connectToWiFi() {
  ...
  else if (wifi_ssid.length() > 0 && wifi_password == ""){
    Serial.print("Connecting to Wi-Fi..");
...
//_____________________________________FIRESTORE FUNCTIONS_________________________
...
void updateSummaryArray(String timeOfTheDay){
  Serial.print("Commit a document (append array)... ");
    ...
    misterOffTime = getMistOff();
        ...
        content.set("values/[0]/mapValue/fields/in_temperature/doubleValue", averageTemp);
        content.set("values/[0]/mapValue/fields/in_humidity/doubleValue", averageHumid);        
...
void updateParameterFields(){
            ...
            content1.set("fields/_in_temperature/doubleValue", temperatureValue);
            content1.set("fields/_in_humidity/doubleValue", humidityValue);
            content1.set("fields/_acidity/doubleValue", pHValue);            
            content1.set("fields/_mistOffTime/doubleValue", misterOnTime);
            content1.set("fields/_mistOnTime/doubleValue", misterOffTime);
            ...
            if (Firebase.Firestore.patchDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath, content1.raw(), "_in_temperature,_in_humidity, _in_humidity, _acidity, _mistOnTime, _mistOffTime, _deviceToken, _lightColor")) {
              Serial.println("");
              //Serial.printf("ok\n%s\n\n", fbdo.payload().c_str());
            } else {
              Serial.println(fbdo.errorReason());
...
//_______________________________GET DATA FROM FIREBASE________________________________________
...
bool readConfigSPIFFS(){
  ...
  const int _misterOnTime = doc["mister_on_time"];
  const int _misterOffTime = doc["mister_off_time"];
...
bool readMisterConfigSPIFFS(){
  ...
  const unsigned long _misterOnTime = doc["mister_on_time"];
  const unsigned long _misterOffTime = doc["mister_off_time"];

  misterOnTime = _misterOnTime;
  misterOffTime = _misterOffTime;

  Serial.println("\nMist Maker on-time: " + String(misterOnTime));
  Serial.println("Mist maker off-time: " + String(misterOffTime));
  Serial.println();
...
bool readAverageParamsSPIFFS(){
  ...
  const double _aHumidity = doc["humidity"];
  const double _aTemperature = doc["temperature"];
  const double _aPH = doc["pH"];

  Serial.println("\nAverage Humidity: " + String(_aHumidity));
  Serial.println("Average Temperature: " + String(_aTemperature));
  Serial.println("Average pH Level: " + String(_aPH));
  Serial.println();
